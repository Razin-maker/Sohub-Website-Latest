          ...
          <div className="flex flex-wrap justify-center gap-4 max-w-4xl mx-auto">
            {resources.map((resource) => (
              <a
                key={resource.label}
                href={resource.href}
                className="group flex items-center gap-4 px-7 py-5 bg-background border border-border rounded-2xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1"
              >
                <resource.icon className="w-5 h-5 text-foreground-muted" />
