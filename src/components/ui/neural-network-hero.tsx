...
function ShaderPlane() {
  ...
  const reduceMotion = useMemo(() => getPrefersReducedMotion(), []);

  useFrame((state) => {
    if (!materialRef.current || !meshRef.current) return;

    // Slightly faster time so the motion is clearly visible
    materialRef.current.iTime = reduceMotion ? 0 : state.clock.elapsedTime * 1.6;
...
function ShaderBackground() {
  ...
  useGSAP(
    () => {
      ...
      gsap.fromTo(
        wrapperRef.current,
        { filter: "blur(16px)", scale: 1.04, autoAlpha: 0 },
        { filter: "blur(0px)", scale: 1, autoAlpha: 1, duration: 1.2, ease: "power3.out" }
      );
    },
    { scope: wrapperRef }
  );
  ...
  return (
    <div ref={wrapperRef} aria-hidden className="absolute inset-0">
      {/* Base split gradient */}
      <div className="absolute inset-0 hero-split-gradient" />

      {/* Shader overlay */}
      <Canvas
        className="absolute inset-0 h-full w-full hero-canvas"
        orthographic
      ...
      {/* Soft highlight to keep things airy */}
      <div className="absolute inset-0 hero-soft-light" />
    </div>
  );
}
...
export default function NeuralNetworkHero({
  ...
  useGSAP(
    () => {
      ...
      if (paraRef.current) gsap.set(paraRef.current, { autoAlpha: 0, y: 10 });
      if (ctaRef.current) gsap.set(ctaRef.current, { autoAlpha: 0, y: 10 });
      if (microRef.current) gsap.set(microRef.current, { autoAlpha: 0, y: 8 });

      if (!reduceMotion) {
        gsap.set(wordEls, { autoAlpha: 0, yPercent: 40, filter: "blur(14px)", scale: 1.03 });
      } else {
        gsap.set(wordEls, { autoAlpha: 1, yPercent: 0, filter: "blur(0px)", scale: 1 });
      }

      const tl = gsap.timeline({ defaults: { ease: "power3.out" } });

      if (badgeRef.current) tl.to(badgeRef.current, { autoAlpha: 1, y: 0, duration: 0.45 }, 0);
      ...
      if (paraRef.current) tl.to(paraRef.current, { autoAlpha: 1, y: 0, duration: 0.55 }, "-=0.45");
      if (ctaRef.current) tl.to(ctaRef.current, { autoAlpha: 1, y: 0, duration: 0.55 }, "-=0.35");
      if (microRef.current) tl.to(microRef.current, { autoAlpha: 1, y: 0, duration: 0.5 }, "-=0.35");
    },
  ...
  return (
    <section ref={sectionRef} className="relative w-full overflow-hidden">
      ...
      <div className="container-main relative z-10 flex min-h-screen flex-col items-center justify-center px-6 text-center">
        {/* Badge */}
        <div ref={badgeRef} className="hero-badge">
          <span className="hero-badge-pill">{badgeLabel}</span>
          <span className="text-sm text-foreground/70">{badgeText}</span>
        </div>

        {/* Title */}
        <h1 className="text-display max-w-5xl text-balance text-foreground">
          {words.map((w, i) => (
            <span
        ...
        <div ref={ctaRef} className="mt-10 flex flex-wrap items-center justify-center gap-4">
          {ctaButtons.map((button) => (
            <a
              ...
              href={button.href}
              className={button.primary ? "hero-btn-primary" : "btn-outline"}
            >
              {button.text}
            </a>
          ))}
        ...
        {/* Micro Details */}
        <div ref={microRef} className="mt-12 flex flex-wrap items-center justify-center gap-x-6 gap-y-3 text-sm text-foreground/60">
