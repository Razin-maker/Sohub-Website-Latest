...
// Modified fragment shader to output orange/white tones instead of dark/blue
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    vec3 finalColor;
    if (luminance < 0.4) {
      finalColor = mix(brandOrange, softCream, luminance / 0.4);
    } else {
      finalColor = mix(softCream, warmWhite, (luminance - 0.4) / 0.6);
    }
    
    // Add subtle variation from original pattern
    finalColor = mix(finalColor, softCream, buf[0].x * 0.3);
